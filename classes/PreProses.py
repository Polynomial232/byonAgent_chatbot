...
class PreProses():
    ...
    def __init__(self, model_folder):
        ...
        with open(f'model/{self.model_folder}/intents.json', 'r', encoding='utf-8') as file:
    ...
    def get_raw_data(self):
        """
            mendapatkan semua data id dan text
        """
        ...
        all_words = [self.stemming(word) for word in all_words if word not in self.ignore_words]
        all_words = sorted(set(all_words))
        ids = sorted(set(ids))

        return ids, all_words
    ...
    def train_data(self):
        ...
        for (pattern_sentence, id) in train_test:
